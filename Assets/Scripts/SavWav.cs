...
public class SavWav : MonoBehaviour
{
    ...
    public void Record()
    {
        ...
        clip.clip = Microphone.Start("", true, 10, audioConfiguration.sampleRate);
    }

    public void Save()
    {
        string filepath = Application.dataPath + "/SaveAudio";
        bool filecheck = false;

        while (!filecheck)
        {
            filename = "/Audio - " + i + ".wav";
            ...
            if (!File.Exists(filepath + filename))
            {
                ...
                {
                    var wav = GetWav(clip.clip, out var length, trim);
